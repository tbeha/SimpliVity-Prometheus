...
parser.add_argument('-p', help='Path /opt/svt/data/')
parser.add_argument('-k', help='key-File')
parser.add_argument('-x', help='XML-File')
...
if __name__ == "__main__":
    ...
    if DEBUG:
        path= os.getcwd()
        keyfile= path+'\\svtdemo.key'
        xmlfile= path+'\\svt380.xml'
        print("Path: ", path)
    ...
    while True:
        try:
            ...
            if int(offset) < 0:
                ...
                if ('f' in monitor) or ('c' in monitor): 
                    clusters = svt.GetCluster({'show_optional_fields':'true','limit':limit,'offset':offset})['omnistack_clusters']
                if ('f' in monitor) or ('n' in monitor):
                    hosts = svt.GetHost({'show_optional_fields':'true','limit':limit,'offset':offset})['hosts']
                if ('f' in monitor) or ('v' in monitor):
                    vms = svt.GetVM({'show_optional_fields':'true','limit':limit,'offset':offset})['virtual_machines']
                if ('f' in monitor):
                    datastores = svt.GetDataStore({'show_optional_fields':'true','limit':limit,'offset':offset})['datastores']                
